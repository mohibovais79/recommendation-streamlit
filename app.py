...
if st.session_state.page == "main":
    ...
    if st.button("Search AI"):
        ...
        if not recommended_properties_prompt.empty:
            ...
            formatted_output_prompt["image"] = formatted_output_prompt.apply(lambda x: get_random_image(), axis=1)
            for index, row in formatted_output_prompt.iterrows():
                st.image(row["image"], width=500)  # Resize image to 100px width
                st.write(f"**Title:** {row['title']}")
                st.write(f"**Address:** {row['displayAddress']}")
...
if st.session_state.page == "search_by_city":
    ...
    if st.button("Search"):
        ...
        if not results.empty:
            ...
            formatted_output["image"] = formatted_output.apply(lambda x: get_random_image(), axis=1)
